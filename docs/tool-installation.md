...
Access the dashboard at `https://192.168.56.104` and confirm it loads.
...
Select: Linux → DEB (amd64) → enter Manager IP `192.168.56.104` → set agent name `secure-agent`.
...
wget https://packages.wazuh.com/4.x/apt/pool/main/w/wazuh-agent/wazuh-agent_4.14.4-1_amd64.deb \
  && sudo WAZUH_MANAGER='192.168.56.104' WAZUH_AGENT_NAME='secure-agent' \
...
Access Splunk Web at `http://192.168.56.105:8000` and complete the first-run setup.
...
./splunk add forward-server 192.168.56.105:9997
...
```yaml
network.host: 192.168.56.108   # listen on host-only interface
...
```yaml
server.host: 192.168.56.108   # accept connections on host-only interface
server.port: 5601
```
...
Open `http://192.168.56.108:5601` in a browser. You will see the **Configure Elastic to get started** screen. Paste the enrollment token.
...
output {
  elasticsearch {
    hosts => ["https://192.168.56.108:9200"]
    user => "elastic"
...
Go to `http://192.168.56.108:5601` → **Stack Management → Data Views → Create data view**
