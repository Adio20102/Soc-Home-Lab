...
|---|---|---|---|---|---|---|---|
| Kali Linux | kali | root | 192.168.56.10 | 2GB | 2 | existing | Attacker + Suricata + SUF + Filebeat |
| Metasploitable 2 | metasploitable | msfadmin | 192.168.56.20 | 512MB | 1 | existing | Victim — no agent |
| Ubuntu S4 | endpoint-01 | endpointadmin | 192.168.56.30 | 1.5GB | 1 | 15GB | Monitored Endpoint — Wazuh Agent |
| Ubuntu S1 | wazuhmachine | admin | 192.168.56.40 | 4GB | 2 | 48GB | Wazuh Manager + Dashboard |
| Ubuntu S2 | splunk-server | splunkadmin | 192.168.56.50 | 4GB | 2 | 50GB | Splunk Enterprise |
| Ubuntu S3 | ir-server | iradmin | 192.168.56.60 | 4GB | 2 | 50GB | TheHive + MISP + Cortex |
| Ubuntu S5 | elk-server | elkadmin | 192.168.56.70 | 4GB | 2 | 50GB | ELK Stack |
...
192.168.56.10  Kali          ──┐
192.168.56.20  Metasploitable  │  All on same /24
192.168.56.30  Ubuntu S4       │  VirtualBox Host-Only
192.168.56.40  Ubuntu S1       │  Adapter: vboxnet0
192.168.56.50  Ubuntu S2       │
192.168.56.60  Ubuntu S3     ──┘
192.168.56.70  Ubuntu S5
```
...
network:
  ...
  ethernets:
    ...
    enp0s8:
      dhcp4: no
      addresses:
        - 192.168.56.XX/24  # replace XX per VM (30, 40, 50, 60, 70)
```
...
Wazuh Agent (S4) → Wazuh Manager S1 → Wazuh Dashboard (https://192.168.56.40)
